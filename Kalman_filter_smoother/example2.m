% -----------------------------------------------------------------------
% Example 2: Solution to a Neoclassical Growth Model via Uhlig's Toolkit.
%
% Obj: Recover FILTERED states + build likelihood with Matlab's KF code
...
% x_t = (k_t, c_t, r_t, z_t)
%
% Explicitely: 1 observable and 4 latent states as AR(1).
% -----------------------------------------------------------------------
...
mdl = ssm(@(params) ParamMap(params, mean_0, cov_0, stateType));


% Run the filter at set parameter values to obtain filtered states:
[X, logL, filteredStates] = filter(mdl, log_deviation, 'params',params_0);

% Store period log_likelihoods
log_likelihoods_seq = [filteredStates.LogLikelihood];
...
long_vector = cell2mat({filteredStates.FilteredStates}'); %960 in length
...
filename = 'filtered_states.png';
saveas(gcf, filename);


%% Save output
% Save the filtered states into a .mat file
save('example2.mat', 'X', 'dates');



%% Figure out LL:
% forecast_obs_cov = [filteredStates.ForecastedObsCov]';
% termB = -0.5 * log(forecast_obs_cov);
% 
% forecast_obs = [filteredStates.ForecastedObs]';
% ferror = log_deviation - forecast_obs;
% termC = -0.5 * (ferror.^2 ./forecast_obs_cov);
% 
% n = size(log_deviation,2);
% termA = -0.5 * n * log(2*pi); % -0.9189
